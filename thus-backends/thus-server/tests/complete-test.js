...
/**
 * 如是(Thus-Note) 后端完整测试脚本
 * 测试所有主要功能模块，包括登录后的所有功能
 */
...
async function testAuth() {
  ...
  if (authToken) {
    try {
      ...
      if (res.status === 200 && res.data.code === "0000") {
        recordTest('获取当前用户信息', 'pass', '用户信息获取成功');
      } else {
        recordTest('获取当前用户信息', 'fail', `状态码: ${res.status}`);
      }
    } catch (error) {
      recordTest('获取当前用户信息', 'fail', error.message);
    }
  } else {
    recordTest('获取当前用户信息', 'skip', '未获取到 access token');
  }
}
...
async function testAI() {
  ...
  try {
    ...
    if (res.status === 200 && res.data.code === "0000") {
      const content = res.data.data.content || '';
      if (content && content.includes && content.includes('未配置')) {
...
async function main() {
  ...
  log('如是(Thus-Note) 后端完整测试', colors.cyan);
  log('='.repeat(60), colors.cyan);
